...
object ShortestPath {
  def main(args: Array[String]) {
    //Spark Context
    val tmpSC = new SparkContext(new SparkConf().setMaster("local[1]").setAppName("ShortestPath"))

    //Read the file describing the graph. Hard-coded to "graph.txt"
    //File Format should be:
    //SrcVertex     DestVertex      EdgeWeight
    var graphFile = tmpSC.textFile("src/main/ShortestPath/graph.txt")
    ...
    var superStep0: RDD[(VertexId, Int)] = initGraph.aggregateMessages(et => Iterator((et.dstId, (et.srcAttr+et.attr))), (value1, value2) => math.min(value1,value2))
    ...
    initGraph.vertices.saveAsObjectFile("src/main/ShortestPath/verticesObject_0")
    initGraph.edges.saveAsObjectFile("src/main/ShortestPath/edgesObject_0")

    //Iterator
    while ((messageCount > 0) &&  (outerCount < 1000)) { //(outerCount < (Int.MaxValue/2))) {

      val iterVertices = tmpSC.objectFile[(VertexId, Int)]("src/main/ShortestPath/verticesObject_"+outerCount)
      val iterEdges = tmpSC.objectFile[Edge[Int]]("src/main/ShortestPath/edgesObject_"+outerCount)
      ...
      while ((messageCount > 0) && (innerCount < 50)) {
        ...
        superStep0 = iterGraph.aggregateMessages(et => Iterator((et.dstId, (et.srcAttr+et.attr))), (value1, value2) => math.min(value1,value2))
        superStep0.cache()
      ...
      //Print answer
      outerCount += 50
      iterGraph.vertices.saveAsObjectFile("src/main/ShortestPath/verticesObject_"+outerCount)
      iterGraph.edges.saveAsObjectFile("src/main/ShortestPath/edgesObject_"+outerCount)
